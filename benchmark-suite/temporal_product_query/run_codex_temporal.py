...
PROMPTS_PATH = SUITE_DIR / "temporal_product_query" / "prompts.json"
SCHEMA_PATH = SUITE_DIR / "temporal_product_query" / "codex_output.schema.json"
...
def build_manifest(args: argparse.Namespace, run_dir: Path) -> dict:
    return {
        "benchmark_id": "temporal_product_query",
        ...
        "repo_state": {
            ...
            "notes": "Codex exec temporal guarded run.",
        },
        "operator_notes": [
            "Token totals are unavailable because this Codex CLI run did not expose a stable token counter.",
            "Temporal guarded condition requires current/history/missing-evidence separation.",
...
def build_prompt(task: dict, workspace: Path) -> str:
    return (
        "You are running a harness-mem temporal product-query benchmark task. "
        "Do not modify files.\n"
        f"Workspace: {workspace}\n"
        f"Task id: {task['task_id']}\n"
        f"Task title: {task['title']}\n"
        "Condition: temporal_guarded. Use repo/file evidence first. You may use "
        "harness-mem read surfaces if available. Separate current truth, "
        "historical/superseded truth, and missing evidence. Do not invent as_of "
        "behavior without stable product evidence.\n\n"
        f"Prompt:\n{task['prompt']}\n\n"
        "Acceptance summary:\n"
...
def write_result(
    ...
    result = {
        ...
        "current_truth": final_payload.get("current_truth", ""),
        "historical_truth": final_payload.get("historical_truth", ""),
        "missing_evidence": final_payload.get("missing_evidence", ""),
    ...
    out = run_dir / "results" / f"{task['task_id']}-temporal_guarded.json"
    out.write_text(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Codex temporal query benchmark.")
    parser.add_argument("--run-name", default="codex-temporal-01")
    parser.add_argument("--workspace", type=Path, default=ROOT)
    parser.add_argument("--model", default="gpt-5.4-mini")
    parser.add_argument(
        "--tasks",
        nargs="+",
        default=["TQ1", "TQ2", "TQ3", "TQ4", "TQ5"],
    )
    ...
    for task_id in args.tasks:
        ...
        prompt_path = run_dir / "notes" / f"{task_id}-temporal_guarded-prompt.md"
        prompt_path.write_text(prompt, encoding="utf-8")
        final_path = run_dir / "transcripts" / f"{task_id}-temporal_guarded-final.json"
        events_path = run_dir / "transcripts" / f"{task_id}-temporal_guarded-events.jsonl"
        ...
        if returncode != 0:
            raise SystemExit(f"{task_id} temporal_guarded: codex exited {returncode}")
