...
PROMPTS_PATH = SUITE_DIR / "runtime_health_observability" / "prompts.json"
SCHEMA_PATH = SUITE_DIR / "runtime_health_observability" / "codex_output.schema.json"
...
def build_manifest(args: argparse.Namespace, run_dir: Path) -> dict:
    return {
        "benchmark_id": "runtime_health_observability",
        ...
        "repo_state": {
            ...
            "notes": "Codex exec runtime health observability run.",
        },
        "operator_notes": [
            "Token totals are unavailable because this Codex CLI run did not expose a stable token counter.",
            "Health guarded condition keeps cost discipline, observability, drift, gates, and false success separate.",
...
def build_prompt(task: dict, workspace: Path) -> str:
    return (
        "You are running a harness-mem runtime health benchmark task. "
        "Do not modify files.\n"
        f"Workspace: {workspace}\n"
        f"Task id: {task['task_id']}\n"
        f"Task title: {task['title']}\n"
        "Condition: health_guarded. Use local repo/file evidence first. Keep "
        "runtime health, version drift, cost discipline, regression gates, "
        "transport diagnosis, and false-success accounting separate. Do not "
        "invent cloud telemetry or obsolete daily CLI fallback guidance.\n\n"
        f"Prompt:\n{task['prompt']}\n\n"
        "Acceptance summary:\n"
...
def write_result(
    ...
    result = {
        ...
        "health_status": final_payload.get("health_status", ""),
        "drift_status": final_payload.get("drift_status", ""),
        "cost_status": final_payload.get("cost_status", ""),
        "regression_gate_status": final_payload.get("regression_gate_status", ""),
        "transport_status": final_payload.get("transport_status", ""),
        "false_success_count": final_payload.get("false_success_count", 0),
    ...
    out = run_dir / "results" / f"{task['task_id']}-health_guarded.json"
    out.write_text(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Codex runtime health benchmark.")
    parser.add_argument("--run-name", default="codex-health-01")
    parser.add_argument("--workspace", type=Path, default=ROOT)
    parser.add_argument("--model", default="gpt-5.4-mini")
    parser.add_argument(
        "--tasks",
        nargs="+",
        default=["RH1", "RH2", "RH3", "RH4", "RH5", "RH6"],
    )
    ...
    for task_id in args.tasks:
        ...
        prompt_path = run_dir / "notes" / f"{task_id}-health_guarded-prompt.md"
        prompt_path.write_text(prompt, encoding="utf-8")
        final_path = run_dir / "transcripts" / f"{task_id}-health_guarded-final.json"
        events_path = run_dir / "transcripts" / f"{task_id}-health_guarded-events.jsonl"
        ...
        if returncode != 0:
            raise SystemExit(f"{task_id} health_guarded: codex exited {returncode}")
