...
PROMPTS_PATH = SUITE_DIR / "generated_knowledge_freshness" / "prompts.json"
SCHEMA_PATH = SUITE_DIR / "generated_knowledge_freshness" / "codex_output.schema.json"
...
def build_manifest(args: argparse.Namespace, run_dir: Path) -> dict:
    return {
        "benchmark_id": "generated_knowledge_freshness",
        ...
        "repo_state": {
            ...
            "notes": "Codex exec generated knowledge freshness run.",
        },
        "operator_notes": [
            "Token totals are unavailable because this Codex CLI run did not expose a stable token counter.",
            "Generated guarded condition requires source-map, freshness, and truth-boundary separation.",
...
def build_prompt(task: dict, workspace: Path) -> str:
    return (
        "You are running a harness-mem generated-knowledge freshness benchmark "
        "task. Do not modify files.\n"
        f"Workspace: {workspace}\n"
        f"Task id: {task['task_id']}\n"
        f"Task title: {task['title']}\n"
        "Condition: generated_guarded. Use repo/file evidence first. Treat "
        "generated prose as generated context, not confirmed truth. Separate "
        "generated claims, source-map support, freshness/invalidation status, "
        "and the confirmed-truth boundary.\n\n"
        f"Prompt:\n{task['prompt']}\n\n"
        "Acceptance summary:\n"
...
def write_result(
    ...
    result = {
        ...
        "generated_claims": final_payload.get("generated_claims", []),
        "source_map_status": final_payload.get("source_map_status", ""),
        "freshness_status": final_payload.get("freshness_status", ""),
        "truth_boundary": final_payload.get("truth_boundary", ""),
    ...
    out = run_dir / "results" / f"{task['task_id']}-generated_guarded.json"
    out.write_text(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Codex generated knowledge benchmark.")
    parser.add_argument("--run-name", default="codex-generated-01")
    parser.add_argument("--workspace", type=Path, default=ROOT)
    parser.add_argument("--model", default="gpt-5.4-mini")
    parser.add_argument("--tasks", nargs="+", default=["GK1", "GK2", "GK3", "GK4", "GK5"])
    ...
    for task_id in args.tasks:
        ...
        prompt_path = run_dir / "notes" / f"{task_id}-generated_guarded-prompt.md"
        prompt_path.write_text(prompt, encoding="utf-8")
        final_path = run_dir / "transcripts" / f"{task_id}-generated_guarded-final.json"
        events_path = run_dir / "transcripts" / f"{task_id}-generated_guarded-events.jsonl"
        ...
        if returncode != 0:
            raise SystemExit(f"{task_id} generated_guarded: codex exited {returncode}")
