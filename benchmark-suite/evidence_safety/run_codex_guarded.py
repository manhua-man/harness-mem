...
PROMPTS_PATH = SUITE_DIR / "evidence_safety" / "prompts.json"
SCHEMA_PATH = SUITE_DIR / "evidence_safety" / "codex_output.schema.json"
...
def build_manifest(args: argparse.Namespace, run_dir: Path) -> dict:
    return {
        ...
        "repo_state": {
            ...
            "notes": "Codex exec guarded evidence-safety run.",
        },
        "operator_notes": [
            "Token totals are unavailable because this Codex CLI run did not expose a stable token counter.",
            "Guarded condition allows repo evidence and memory reads only when needed; every visible call is recorded.",
...
def build_prompt(task: dict, workspace: Path) -> str:
    return (
        "You are running a harness-mem evidence-safety benchmark task. "
        "Do not modify files.\n"
        f"Workspace: {workspace}\n"
        f"Task id: {task['task_id']}\n"
        f"Task title: {task['title']}\n"
        "Condition: guarded. Use repo/file evidence first. You may use "
        "harness-mem read surfaces only if needed for current memory truth. "
        "Record every visible memory and repo call. Keep evidence found separate "
        "from the claim safely supported.\n\n"
        f"Prompt:\n{task['prompt']}\n\n"
        "Acceptance summary:\n"
...
def write_result(
    ...
    result = {
        ...
        "evidence_found": final_payload.get("evidence_found", []),
        "safe_claim": final_payload.get("safe_claim", ""),
    ...
    out = run_dir / "results" / f"{task['task_id']}-guarded.json"
    out.write_text(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Codex evidence-safety benchmark.")
    parser.add_argument("--run-name", default="codex-guarded-01")
    parser.add_argument("--workspace", type=Path, default=ROOT)
    parser.add_argument("--model", default="gpt-5.4-mini")
    parser.add_argument("--tasks", nargs="+", default=["E1", "E2", "E3", "E4", "E5"])
    ...
    for task_id in args.tasks:
        ...
        prompt_path = run_dir / "notes" / f"{task_id}-guarded-prompt.md"
        prompt_path.write_text(prompt, encoding="utf-8")
        final_path = run_dir / "transcripts" / f"{task_id}-guarded-final.json"
        events_path = run_dir / "transcripts" / f"{task_id}-guarded-events.jsonl"
        ...
        if returncode != 0:
            raise SystemExit(f"{task_id} guarded: codex exited {returncode}")
