...
当前实现已有受控自动化：conversation-level autopilot、opt-in host hook / scheduler
trigger、默认关闭 Auto Dream。默认常驻后台不做；truth 不静默改。`suggest_*`
只是受控 agent/runtime 流程中的候选写入接口，不能绕过 candidate / review /
supersede / ledger。
