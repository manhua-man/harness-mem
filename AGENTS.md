...
- Python 包：`harness-mem`，当前发布版本 `0.9.24`；版本真值在 `pyproject.toml` 与 `harness_mem/__init__.py`。
...
- **当前兼容边界：** `0.9.22` 发布路径（并由 `0.9.23` 与 `0.9.24` 延续）已分离干净当前知识、临时 job 材料和最小来源/undo 数据；旧 `MemoryEntry` 兼容行不会因升级自动迁移、删除或改写。
...
| SQLite 当前知识收敛 | `docs/roadmap/knowledge-truth-separation.md` | `0.9.22` 发布实现（`0.9.23` 与 `0.9.24` 延续）；普通运行不迁移真实旧记忆，已授权收敛必须项目隔离、来源重验且可逆 |
