...
/** 十维：harness-mem v5.8 vs 各维参考项目峰值 */
const TEN_DIMENSIONS = [
  ...
  },
  {
    ...
    peak: 93,
    peakProject: "hindsight",
    note: "full=60 / minimal=28；默认 full",
  },
  {
    id: "temporal",
    label: "⑥ 时序",
    ...
    peak: 91,
    peakProject: "hindsight",
    note: "temporal_query + 主路径 drilldown",
  },
  {
    id: "wiki",
    label: "⑦ Wiki",
    ...
    peak: 97,
    peakProject: "meta-kb",
    note: "generated claim drilldown + incremental compile",
  },
  {
    id: "cost",
    label: "⑧ 成本",
  ...
  },
  {
    ...
    note: "guided maintenance profiles；opt-in",
  },
  {
    id: "evidence",
    label: "⑩ 证据",
...
const RELEASE_DELTAS = [
  ...
  { slice: "v5.7 Temporal Drilldown", impact: "⑥ 时序 +4", detail: "主路径 temporal metadata / temporal_query drilldown" },
  { slice: "v5.8 MCP Profile", impact: "⑤ MCP +1", detail: "full=60 / minimal=28；默认 full" },
  { slice: "v5.8 Maintenance Profile", impact: "⑨ 维护 +6", detail: "profile dry-run summary；不自动运行 dream/metabolism" },
  { slice: "v5.8 Incremental Compile", impact: "⑦ Wiki +4", detail: "source hash skip；不宣称性能收益" },
];
  ...
  return (
    <Stack gap={20} style={{ padding: 20, maxWidth: 980, margin: "0 auto" }}>
      ...
      <Callout tone="warning">
        能力雷达，不是总分榜。蓝柱 = harness-mem v5.8；橙柱 = 该维参考项目峰值（天花板，非 hm 得分）。不能把
      ...
      <Stack gap={6}>
        ...
        <Text tone="tertiary" size="small">
          主观分 0–100 · 参考源码 ../upstreams/harness-mem/ · v5.3–v5.8 已计入 DX / maintenance / outcome-loop / temporal / MCP profile
        </Text>
      </Stack>
      <Card>
        <CardBody>
      ...
      <Card>
        <CardHeader>分维明细与 v5.8 依据</CardHeader>
        <CardBody style={{ padding: 0 }}>
          <Table
            headers={["维", "hm", "峰值", "峰值项目", "差", "v5.8 备注"]}
      ...
      <Card>
        <CardHeader>v5.3–v5.8 对本矩阵的影响</CardHeader>
        <CardBody style={{ padding: 0 }}>
          <Table
            headers={["切片", "评分影响", "说明"]}
            rows={RELEASE_DELTAS.map((r) => [r.slice, r.impact, r.detail])}
          />
        </CardBody>
      </Card>
