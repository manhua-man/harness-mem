...
- `distilled`: session note is written, stable knowledge is promoted, and any repo-local project-rule decision is complete.
...
5. Decide whether any promoted lesson should also update an existing repo-local guidance file in the active project.
...
- `/hm:verify-entry <session-id|keyword>`: pull matching knowledge entries and focused recheck questions.
