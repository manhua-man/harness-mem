---
name: hm-distill
description: Run the harness-mem distill daily action when the user invokes hm-distill.
---
...
- 项目名：先调 MCP `get_project_status(project_root=<当前工作区>, host_client="grok")`，没有就问用户
- 会话数：默认 5
...
1. **确认项目**
   ...
   - 否则调 MCP `get_project_status(project_root=<当前工作区>, host_client="grok")` 读取 active project，并幂等检查项目 Hook
...
4. **收尾当前 job**
   ...
   候选判断必须复用 shared auto-review policy，而不是在命令文档里手写另一套规则。安全候选自动进入 truth layer；其余候选自动终结。`/hm-review` 是事后 audit、纠错和 undo 入口，不是日常晋升闸门。
