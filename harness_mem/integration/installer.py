"""Generic IDE hook installer for the v2.4.3 maintenance CLI (Req 5, 6).
...
Boundary self-check (Req 7, design "Boundary enforcement"): after substitution
...
template can never produce a violating artifact on disk — defense-in-depth that
is intentionally redundant with the Req 7 test suite.
"""
...
# Default documentation pointer baked into generated hook headers (Req 8.4).
# Task 6 finalizes the canonical location; callers may override per invocation.
DEFAULT_DOC_POINTER = "docs/cli/v2.4.md"
...
def _assert_boundary(rendered: str) -> None:
    """Enforce the v2.4.0 Req 10.2 hook boundary on a rendered body.
...
def install_hook(
    *,
    ...
    Args:
        ...
        project_root: Repo whose absolute path is baked into the hook (Req 5.2).
        force: Overwrite an existing ``target_path`` when ``True`` (Req 5.7).
        harness_mem_version: Version string for the comment header (Req 5.8).
        generated_at: Generation timestamp for the comment header (Req 5.8).
        doc_pointer: Documentation path baked into the header (Req 8.4).
    ...
    Raises:
        ...
        RuntimeError: the rendered body violates the hook boundary (Req 7).
