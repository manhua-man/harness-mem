...
GENERATED_WIKILINK_GRAPH_FILENAME = "wikilink-graph.json"
...
async def knowledge_cache_health(
    ...
    *,
    ...
    return {
        ...
        "wikilink_node_count": int(generated_counts.get("wikilink_nodes", 0) or 0),
        "wikilink_edge_count": int(generated_counts.get("wikilink_edges", 0) or 0),
...
async def rebuild_wiki_bridge(
    ...
    *,
    ...
    tracked_outputs = [
        ...
        GENERATED_WIKILINK_GRAPH_FILENAME,
    ]
    ...
    wikilink_graph = _build_wikilink_graph(
        project_name=project_name,
        profile=profile,
        project_root=project_root,
        claims=valid_claims,
    ...
    wikilink_graph_path = paths.generated_root / GENERATED_WIKILINK_GRAPH_FILENAME
    ...
    wikilink_graph_path.write_text(
        json.dumps(wikilink_graph, indent=2),
        encoding="utf-8",
    )
    ...
    generated_index_payload = {
        ...
        "counts": {
            ...
            "sources": len(sources),
            "wikilink_nodes": len(wikilink_graph.get("nodes", [])),
            "wikilink_edges": len(wikilink_graph.get("edges", [])),
        },
        "compile_metrics": compile_metrics,
        "freshness": freshness,
    ...
    return {
        ...
        "wikilink_graph_path": str(wikilink_graph_path),
        ...
        "claim_diff": claim_diff["summary"],
        "wikilink_node_count": len(wikilink_graph.get("nodes", [])),
        "wikilink_edge_count": len(wikilink_graph.get("edges", [])),
        "tracked_outputs": tracked_outputs,
        "incremental": bool(incremental),
        "skipped_source_count": 0,
    }
...
def _extract_wikilinks(text: str) -> list[tuple[str, str]]:
    ...
    for match in _WIKILINK_PATTERN.finditer(text):
        ...
        if not target:
            continue
        links.append((target, label or target))
    return links


def _wikilink_node_id(target: str) -> str:
    slug = _target_slug(target)
    return f"wikilink:{slug}"


def _curated_doc_node_id(curated_path: str) -> str:
    return f"curated-doc:{_target_slug(curated_path)}"
...
def _build_wikilink_graph(
    *,
    ...
    """Compile wikilink edges from curated docs and generated claims (non-canonical)."""
    ...
    def _ensure_node(
        ...
        *,
        ...
        if existing is None:
            ...
            return
        merged_sources = sorted(set(existing.get("source_ids", [])) | set(payload["source_ids"]))
        nodes[node_id] = {
            **existing,
            "source_ids": merged_sources,
        }

    def _add_edge(*, from_id: str, to_id: str, source_id: str, quote: str | None = None) -> None:
        key = (from_id, to_id, source_id)
        if key in edge_keys:
            return
        edge_keys.add(key)
        edges.append(
            {
                "edge_id": f"{from_id}->{to_id}:{len(edges) + 1}",
                "from": from_id,
                "to": to_id,
                "edge_kind": "wikilink",
                "source_id": source_id,
    ...
    for curated_path in (profile.curated_doc_paths if profile else []):
        ...
        if not resolved.exists() or not resolved.is_file():
            continue
        source_id = f"curated-doc://{_normalize_source_label(curated_path)}"
        from_id = _curated_doc_node_id(curated_path)
        _ensure_node(
            from_id,
            label=Path(curated_path).name,
            node_kind="curated_doc",
            source_ids=(source_id,),
        )
        text = resolved.read_text(encoding="utf-8")
        for target, label in _extract_wikilinks(text):
            to_id = _wikilink_node_id(target)
            _ensure_node(to_id, label=label, node_kind="wikilink_target")
            _add_edge(from_id=from_id, to_id=to_id, source_id=source_id, quote=target)

    for claim in claims:
        from_id = _claim_node_id(claim.claim_id)
        source_ids = tuple(
            str(ref.get("source_id") or "")
            for ref in claim.source_refs
        ...
        for target, label in _extract_wikilinks(claim.text):
            to_id = _wikilink_node_id(target)
            _ensure_node(to_id, label=label, node_kind="wikilink_target")
            primary_source = source_ids[0] if source_ids else claim.claim_id
            _add_edge(from_id=from_id, to_id=to_id, source_id=primary_source, quote=target)

    return {
        "project_name": project_name,
        "authority": COMPILED_AUTHORITY,
        "updated_at": _utc_now().isoformat(),
        "compiler_version": "v5.13",
        "nodes": sorted(nodes.values(), key=lambda item: str(item.get("node_id"))),
        "edges": edges,
    }
