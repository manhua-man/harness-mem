...
_METABOLISM_RUN_LIST_DEFAULT_LIMIT = 50
_METABOLISM_RUN_LIST_MAX_LIMIT = 200
_VALID_METABOLISM_RUN_KINDS: frozenset[str] = frozenset(
    {"preview", "metabolism"}
)


def _clamp_maintenance_limit(value: int, *, default: int, maximum: int) -> int:
    try:
        parsed = int(value)
...
def tool_list_reflection_jobs(
    ...
    """Read reflection/dream job audit rows for the maintenance profile."""
    if status is not None and status not in _VALID_REFLECTION_JOB_STATUSES:
        valid = ", ".join(sorted(_VALID_REFLECTION_JOB_STATUSES))
        return {"success": False, "error": f"Invalid status: {status!r}. Valid values: {valid}."}
    if kind is not None and kind not in _VALID_REFLECTION_JOB_KINDS:
        valid = ", ".join(sorted(_VALID_REFLECTION_JOB_KINDS))
        return {"success": False, "error": f"Invalid kind: {kind!r}. Valid values: {valid}."}

    clamped = _clamp_maintenance_limit(
        limit,
        default=_REFLECTION_JOB_LIST_DEFAULT_LIMIT,
        maximum=_REFLECTION_JOB_LIST_MAX_LIMIT,
    )
    jobs = _get_backend().reflection_job_store.list(
        project_name=project_name,
        status=status,
        kind=kind,
        limit=clamped,
...
def tool_get_reflection_job(job_id: str) -> dict:
    """Read one reflection/dream job audit row for the maintenance profile."""
    job = _get_backend().reflection_job_store.get(job_id)
    if job is None:
        return {"success": False, "error": f"Reflection job not found: {job_id}"}
    return {"success": True, "job": job.to_dict()}


def tool_list_metabolism_runs(
    project_name: str | None = None,
    kind: str | None = None,
    limit: int = _METABOLISM_RUN_LIST_DEFAULT_LIMIT,
) -> dict:
    """Read persisted metabolism audit rows without triggering metabolism."""
    if kind is not None and kind not in _VALID_METABOLISM_RUN_KINDS:
        valid = ", ".join(sorted(_VALID_METABOLISM_RUN_KINDS))
        return {"success": False, "error": f"Invalid kind: {kind!r}. Valid values: {valid}."}
    ...
    clamped = _clamp_maintenance_limit(
        limit,
        default=_METABOLISM_RUN_LIST_DEFAULT_LIMIT,
        maximum=_METABOLISM_RUN_LIST_MAX_LIMIT,
    )
    runs = asyncio.run(
        _get_backend().structured_store.list_metabolism_runs(
            resolved,
            kind=kind,
            limit=clamped,
        )
...
def tool_health_summary(project_name: str | None = None) -> dict:
    """Read runtime health diagnostics for the maintenance profile."""
    resolved = (project_name or "").strip() or get_active_project()
    if not resolved:
        return {
            "success": False,
...
def tool_surface_cost_report(
    ...
    """Read MCP surface cost diagnostics for the maintenance profile."""
    return {
        "success": True,
        **surface_cost_report(
            _observer_data_dir(),
...
def build_tool_handlers() -> dict[str, Callable[..., dict[str, Any]]]:
    ...
    return {
        ...
        "list_metabolism_runs": tool_list_metabolism_runs,
