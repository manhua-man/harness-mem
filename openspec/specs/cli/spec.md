...
`purge` / `maintenance` / `config` / `integration`。历史日常子命令（`use`、
`wake`、`search`、`ingest`、`status`、`profile`、`timeline`、`candidates`、
`confirm`、`reject`、`correct`、`handoff`、`search-raw`、`trace-relations`、
`distill` 等）不属于当前 CLI surface。
## Requirements
