...
def test_current_canvases_match_0910_automation_and_live_status_contract() -> None:
    ...
    readiness = Path(
        "docs/canvases/harness-mem-readiness-0-9-10.canvas.tsx"
    ).read_text(encoding="utf-8")
...
def test_repo_local_duplicate_distill_runtime_is_removed() -> None:
    ...
    assert not Path("code/tools/hm-distill/lib").exists()
    assert not Path("code/tools/hm-distill/bin").exists()
...
def test_active_docs_do_not_restore_legacy_distill_product_name() -> None:
    ...
    paths = [
        ...
        Path("CHANGELOG.md"),
        *Path("docs").rglob("*.md"),
        *Path("code/tools/hm-distill").rglob("*.md"),
        *Path("code/plugins/harness-mem").rglob("*.md"),
        *Path("docs/canvases").rglob("*.tsx"),
    ]
