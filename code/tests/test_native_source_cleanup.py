...
def test_missing_source_before_atomic_claim_is_not_reported_as_deleted(
    tmp_path: Path,
) -> None:
    root = tmp_path / "sessions"
    ...
    moved_path = source_path.with_suffix(".reactivated")
    source_path.rename(moved_path)

    result = apply_native_source_cleanup(plan)

    assert result["status"] == "partial_failure"
    assert result["reason_codes"] == ["native_source_missing_before_claim"]
