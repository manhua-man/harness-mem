...
def test_wait_for_autonomous_receipt_reports_deferred_as_hook_failure(tmp_path) -> None:
    receipt = {
        "state": "deferred",
        "trigger_id": "turn-1",
        "dispatch_generation": "generation-1",
    ...
    assert code == ExitCode.HOOK_FAILED
    assert json.loads(stdout_payload) == {
        "action": "post-turn-maintenance",
        "dispatch_generation": "generation-1",
        "error": {"kind": "provider_busy", "message": "retry later"},
        "job": "job-1",
        "job_id": "job-1",
        "receipt": str(receipt_path),
        "state": "deferred",
