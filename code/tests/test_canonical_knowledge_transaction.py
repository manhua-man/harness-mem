...
def test_delete_is_scoped_to_project_when_entity_ids_collide(tmp_path):
    store = LocalStructuredStore(tmp_path)
    project_a = {**_entry("shared-id", "Project A truth"), "project_name": "project-a"}
    project_b = {**_entry("shared-id", "Project B truth"), "project_name": "project-b"}
    store.write_record_payload("knowledge_entries", "shared-id", project_a)
    store.write_record_payload("knowledge_entries", "shared-id", project_b)
    digest_a = store.record_payload_sha256(
        "knowledge_entries", "shared-id", project_name="project-a"
    )

    store.apply_canonical_payload_transaction(
        idempotency_key="project-a:delete-shared",
        mutations=[
            {
                "operation": "delete",
                "collection": "knowledge_entries",
    ...
    remaining = store.list_record_payloads("knowledge_entries")
    assert [(item["project_name"], item["statement"]) for item in remaining] == [
        ("project-b", "Project B truth")
    ]
    store.close()
