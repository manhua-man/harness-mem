...
When a project ships across multiple clients (e.g. Claude Code + Codex CLI
...
- Claude Code: dogfood baseline, slash commands assumed
- Codex CLI: tests "decoupled from Claude Code" promise
...
- An audit identifies "we promise cross-client support but only test in
  Claude Code" → use `multi-client-field-test` to build the validation
