...
records the `0.9.x` compatibility statuses. In the current `0.9.28` source,
...
- extraction discovers every independent claim supported by the source; it does not decide final
...
and named targets and performs the knowledge/source change in one transaction
on the existing `canonical.sqlite` file.

| Decision | Durable effect |
|---|---|
| `add` | Add source-backed atomic current knowledge |
| `refine` | Replace one or more named current items with more precise items |
| `confirm` | Keep one named current item; add no duplicate |
| `replace` | Replace one or more named current items with a newer conclusion |
...
`confirm` requires exactly one current SQLite target. `refine` and `replace`
may name one or more current targets and replace them in one transaction.
...
- **Review** handles human correction, replacement, and deletion.
...
and the same SQLite current-memory change. Neither treats an old database
...
## Normal retrieval and requested details
...
An explicit source request may join a knowledge item to its minimal source
locator, finite receipt, or Session Note/Packet. An old result explains what
happened then; it cannot prove the underlying
repository, configuration, web page, or API still supports the claim now.

Replacing knowledge deletes the old item and writes the new item. Deleting
knowledge removes it. `0.9.28` does not keep knowledge versions, archived
knowledge copies, knowledge change records, or undo.
...
| terminal add/refine/confirm/replace/no_write/reject | Clean detail after knowledge/no-write, Note/Packet, and receipt are durable |
...
memory and is retained only while retry or unfinished work needs it.

## `0.9.x` compatibility statuses

The compatibility model introduced in `0.9.20` and carried through `0.9.28`
...
| `auto_confirmed` | Legacy row | Compatibility data only; never current memory or a search result |
| `provisional` | Legacy risky row | Compatibility data only; never current memory or a search result |
| `user_confirmed` | Legacy user-confirmed row | Compatibility data only; never current memory or a search result |
| `superseded` | Legacy replaced row | Compatibility data only; never current memory or a search result |
...
- Source cleanup remains receipt-first, policy-authorized, session-scoped, and
  stops when safety cannot be proved.
...
- [Recall details](recall-audit.md)
