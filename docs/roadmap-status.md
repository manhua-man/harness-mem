...
| `harness_mem/__init__.py` | `3.2.0` |
| `CHANGELOG.md` | 已有 `3.2.0` 段；`Unreleased` 当前为空 |

## 产品基线

当前收口基线是 v3.2.0：v1.5 baseline、v1.6 persistent vectors / bucket budget、
...
truth-sync 的 release train、v3.1 Auto Dream Memory Maintenance，以及 v3.2
Generated Knowledge Compiler + Basic Freshness 都已落地。

当前版本 v3.2.0 已发布 Generated Knowledge Compiler + Basic Freshness：source map、
...
| 切片 | 状态 | 目标 | 文档 |
|---|---|---|---|
| v3.3.x Temporal Query and Supersede Explainability | 规划中，未实现 | temporal truth 可查询；current/history/as_of、supersede timeline、temporal evaluation；多跳图只做后置 proof | `docs/roadmap-v33.md` |
...
| v3.3.x | Temporal Query and Supersede Explainability：current/history/as_of 查询、supersede timeline、abstention、temporal evaluation；多跳图后置 | `docs/roadmap-v33.md` |
...
从 v1.5 baseline 到 v3.2 Generated Knowledge Compiler，主实现路线已经按一个版本一个文档重切并连续收口。
...
source map / atomic claim / citation validation / incremental metrics 都已落地。
...
后续 v3.3-v3.4 依次规划 temporal query / supersede explainability、runtime health / cost discipline / regression gates；它们是 v3.2 后的设计方向，不代表已发布能力。
