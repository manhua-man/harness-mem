...
| 来源 | 值 |
|---|---|
| `pyproject.toml` | `2.9.20` |
| `harness_mem/__init__.py` | `2.9.20` |
| `CHANGELOG.md` | 已有 `2.9.20` 段；`Unreleased` 当前为空 |

当前收口基线是 v2.9.20：v2.5.0–v2.5.2 的 context assembly / wake renderer /
...
> **v2.9.20 发版状态（2026-06-03）**：版本号已 bump 到 `2.9.20`。v2.9 在保持
...
> `list_candidates -> auto-review / confirm / reject` 的旧主链画法。
...
| v2.9.20 | 当前版本 | `README.md`、`tests/test_readme_distill_truth.py`、OpenSpec `v2920-readme-distill-workflow-truth-sync` | README 的 distill workflow 图现在直接指向 `auto_review_candidates(apply=true)`；不再把 `list_candidates -> auto-review / confirm / reject` 画成 shipped 主链。 |
