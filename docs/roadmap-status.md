...
| 来源 | 值 |
|---|---|
| `pyproject.toml` | `2.9.48` |
| `harness_mem/__init__.py` | `2.9.48` |
| `CHANGELOG.md` | 已有 `2.9.48` 段；`Unreleased` 当前为空 |

当前收口基线是 v2.9.48：v1.5 baseline、v1.6 persistent vectors / bucket budget、
...
v2.9.0–v2.9.48 这一整条从 `/hm:prd-sync` 起步、随后扩成 maintenance / triage /
truth-sync 的 release train 都已落地。

> **v2.9.48 发版状态（2026-06-03）**：版本号已 bump 到 `2.9.48`。v2.9 在保持
...
| v2.9.48 | 当前版本 | `docs/v2-user-test-packet.md`、`tests/test_v2_user_test_packet_contract_source_truth.py`、OpenSpec `v2948-user-test-packet-openspec-source-hierarchy-sync` | `v2-user-test-packet` 现在明确默认先看 `openspec/specs/...` 作为主 spec 真值；只有确有 active change proposal 时，才下钻 `openspec/changes/<change>/specs/...`。 |
