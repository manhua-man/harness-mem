...
| 来源 | 值 |
|---|---|
| `pyproject.toml` | `2.9.6` |
| `harness_mem/__init__.py` | `2.9.6` |
| `CHANGELOG.md` | 已有 `2.9.6` 段；`Unreleased` 当前为空 |

当前收口基线是 v2.9.6：v2.5.0–v2.5.2 的 context assembly / wake renderer /
...
> stale-surface guardrail、shell completion、MCP/user-test collateral 一起
...
| v2.9.6 | 当前收口基线 | `openspec/specs/mcp/spec.md` sync、`docs/v2-user-test-packet.md` sync、OpenSpec `v296-maintenance-surface-collateral-sync` | MCP 主 spec 与用户测试包现在都承认 `config` / `integration` 属于当前维护 CLI surface。 |
