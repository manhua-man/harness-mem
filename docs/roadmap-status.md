...
| 来源 | 值 |
|---|---|
| `pyproject.toml` | `2.9.19` |
| `harness_mem/__init__.py` | `2.9.19` |
| `CHANGELOG.md` | 已有 `2.9.19` 段；`Unreleased` 当前为空 |

当前收口基线是 v2.9.19：v2.5.0–v2.5.2 的 context assembly / wake renderer /
...
> **v2.9.19 发版状态（2026-06-03）**：版本号已 bump 到 `2.9.19`。v2.9 在保持
...
> per-item `list_candidates` + `confirm_*` / `reject_*` 写成默认 distill 路径。
...
| v2.9.19 | 当前版本 | `docs/best-practices.md`、`tests/test_best_practices_auto_review_truth.py`、OpenSpec `v2919-best-practices-auto-review-truth-sync` | `best-practices` 现在把 `auto_review_candidates(project_name=<project>, apply=true)` 写成默认 distill review surface；`list_candidates` 只保留给显式 drilldown/recheck。 |
