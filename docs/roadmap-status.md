...
| 来源 | 值 |
|---|---|
| `pyproject.toml` | `2.9.51` |
| `harness_mem/__init__.py` | `2.9.51` |
| `CHANGELOG.md` | 已有 `2.9.51` 段；`Unreleased` 当前为空 |

当前收口基线是 v2.9.51：v1.5 baseline、v1.6 persistent vectors / bucket budget、
...
v2.9.0–v2.9.51 这一整条从 `/hm:prd-sync` 起步、随后扩成 maintenance / triage /
truth-sync 的 release train 都已落地。

> **v2.9.51 发版状态（2026-06-03）**：版本号已 bump 到 `2.9.51`。v2.9 在保持
...
| v2.9.51 | 当前版本 | `docs/README.md`、`tests/test_docs_readme_truth_authority_sync.py`、OpenSpec `v2951-docs-readme-truth-authority-sync` | docs 文档索引入口现在也明确把当前发版状态、已完成切片和未做边界 authority 指向 `roadmap-status.md` 与 `CHANGELOG.md`；各版本 roadmap 只作为设计与历史决策链，不单独充当当前实现真值。 |
