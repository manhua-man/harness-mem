...
covered by deterministic and isolated runtime probes. 0.9.15 now isolates raw
sources and internal audit metadata: normal search and wake return clean
canonical memory prose unless an explicit deep/full diagnostic read is used.
Governed historical-data cutover has not started.
