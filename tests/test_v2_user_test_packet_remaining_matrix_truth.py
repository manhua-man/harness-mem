...
def test_v2_user_test_packet_explicitly_records_remaining_strong_evidence_gaps() -> None:
    ...
    assert "剩下没补齐的仍是更强的 live client scenarios" in roadmap_status
    assert "`S5/S7`、UI 级 cross-client pair" in roadmap_status
    assert "`harness_mem/integration` 工作区上的真实 Cursor packet scenario run log" in roadmap_status
    assert "UI 级 cross-client pair、Claude 自动化读端、或 integration-workspace Cursor packet run log 已完成" in roadmap_status
