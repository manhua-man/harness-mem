...
def test_v32_is_implemented_and_v33_v34_remain_planning() -> None:
    ...
    assert (
        "| v3.3.x Temporal Query and Supersede Explainability | 规划中，未实现 |"
        in roadmap_status
    )
    assert (
    ...
    assert "| v3.2.0 | 当前版本 |" in roadmap_status
    ...
    assert "| v3.3.x | Temporal Query and Supersede Explainability" in roadmap_status
    assert "| v3.4.x | Runtime Health, Cost Discipline, and Regression Gates" in roadmap_status
    assert "temporal query / supersede explainability" in roadmap_status
    ...
    assert "> 状态：规划中，未实现。" in roadmap_v33
