...
- 项目名：先调 MCP `get_project_status(project_root=<当前工作区>, host_client="opencode")`，没有就问用户
- 会话数：默认 5
...
1. **确认项目**
   ...
   - 否则调 MCP `get_project_status(project_root=<当前工作区>, host_client="opencode")` 读取 active project，并幂等检查项目 Hook
