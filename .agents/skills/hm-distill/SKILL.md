...
- 项目名：先调 MCP `get_project_status(project_root=<当前工作区>, host_client="codex")`，没有就问用户
- 会话数：默认 5
...
4. **收尾当前 job**
   ...
   候选判断必须复用 shared auto-review policy，而不是在 Skill 里手写另一套规则。安全候选自动进入 truth layer；其余候选自动终结。`$hm-review` 是事后 audit、纠错和 undo 入口，不是日常晋升闸门。
